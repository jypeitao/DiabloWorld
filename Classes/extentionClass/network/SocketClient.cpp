    ...
    while (This->m_iState == SocketClient_WAIT_CONNECT && !This->connectServer())
    {
        ...
        else
        {
            ...
            //		error.append(This->m_host).append(":").append(tmp);
            {
                ...
                This->m_receivedMessageQueue.push(
                        constructErrorMessage(TYPE_SELF_DEINE_MESSAGE_CONNECT_FAIL, errno, error));
            }
        ...
        if (This->m_iState == SocketClient_OK)
        {
            ...
            if (sendBuff.getPosition() > 0)
            {
                ...
                if (ret == -1)
                {
                    ...
                    MyLock lock(&This->m_sendqueue_mutex);
                    This->m_receivedMessageQueue.push(
                            constructErrorMessage(TYPE_SELF_DEINE_MESSAGE_CANNOT_SEND_MESSAGE,
                                    errno, err));
                    return ((void *) 0);
                }
                else
            ...
            while (This->m_iState != SocketClient_DESTROY && This->m_sendMessageQueue.size() > 0)
            {
                ...
                if (msg->datalength() + sendBuff.getPosition() > sendBuff.getLimit())
                {
                    ...
                    MyLock lock(&This->m_sendqueue_mutex);
                    This->m_receivedMessageQueue.push(
                            constructErrorMessage(TYPE_SELF_DEINE_MESSAGE_CANNOT_SEND_MESSAGE, 0,
                                    "发送缓冲器已满，您的网络环境好像出现了问题！"));
                    return ((void *) 0);
                }
...
//		int ret = select(This->m_hSocket+1,&fdRead,NULL,NULL,NULL);
        ...
        if (ret == -1)
        {
            ...
            else
            {
                ...
                MyLock lock(&This->m_sendqueue_mutex);
                This->m_receivedMessageQueue.push(
                        constructErrorMessage(TYPE_SELF_DEINE_MESSAGE_CONNECT_TERMINATE, errno,
                                "连接异常中断"));
                return ((void *) 0);
            }
        }
        else if (ret == 0)
        {
            ...
            if (g_bcheckReceivedMessage)
            {
                        ...
                        && now.tv_sec - lastHintUserTime.tv_sec > hintTimeInSeconds)
                {
                    ...
                    This->m_receivedMessageQueue.push(
                            constructErrorMessage(TYPE_SELF_DEINE_MESSAGE_RECONNECT_FORCE, 0,
                                    "您的网络已经出问题了！"));
                        ...
                        && now.tv_sec - lastHintUserTime.tv_sec > hintTimeInSeconds)
                {
                    ...
                    MyLock lock(&This->m_sendqueue_mutex);
                    This->m_receivedMessageQueue.push(
                            constructErrorMessage(TYPE_SELF_DEINE_MESSAGE_RECONNECT_HINT, 0,
                                    "您的网络好像出问题了！"));
                }
                        ...
                        && now.tv_sec - lastHintUserTime.tv_sec > hintTimeInSeconds)
                {
                    ...
                    MyLock lock(&This->m_sendqueue_mutex);
                    This->m_receivedMessageQueue.push(
                            constructErrorMessage(TYPE_SELF_DEINE_MESSAGE_IDLE_TIMEOUT, 0,
                                    "您的网络好像出问题了！"));
                }
            }
            else
        ...
        else if (ret > 0)
        {
            if (FD_ISSET(This->m_hSocket,&fdRead))
            {
                ...
                {
                    iRetCode = recv(This->m_hSocket,
                            recvBuff->getBuffer() + recvBuff->getPosition(), recvBuff->remaining(),
                            0);
                }
                ...
                else if (iRetCode == 0 && recvBuff->remaining() > 0)
                {
                    ...
                    This->m_receivedMessageQueue.push(
                            constructErrorMessage(TYPE_SELF_DEINE_MESSAGE_SERVER_CLOSE_CONNECTION,
                                    errno, "服务器主动关闭连接!"));
                ...
                else
                {
                    ...
                    while (recvBuff->remaining() > tmpOffset)
                    {
                        ...
                        if (recvBuff->remaining() + tmpOffset >= length)
                        {
                            ...
                            log("接收队列长度:::: %lu", sizeof(This->m_receivedMessageQueue));
                        ...
                        else if (length > recvBuff->getCapacity())
                        {
                            ...
                            This->m_receivedMessageQueue.push(
                                    constructErrorMessage(TYPE_SELF_DEINE_MESSAGE_CONNECT_TERMINATE,
                                            0, "数据包太大，连接中断！"));
                            return ((void *) 0);
                        }
                        else
...
void* ThreadListen(void* p)
{
    ...
    while (serv->isStartedServer) //服务器端要一直运行
    {
        ...
        int new_server_socket = accept(serv->server_socket, (struct sockaddr*) &client_addr,
                &length);
